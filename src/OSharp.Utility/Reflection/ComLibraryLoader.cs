        ...
        public object CreateObjectFromPath(string dllPath, Guid clsid, bool setSearchPath, bool comFallback)
        {
            if (File.Exists(dllPath) && (_preferURObjects || !comFallback))
            {
                ...
                if (_lib != IntPtr.Zero)
                {
                    ...
                    if (ptr != IntPtr.Zero)
                    {
                        DllGetClassObjectInvoker invoker = Marshal.GetDelegateForFunctionPointer(ptr, typeof(DllGetClassObjectInvoker)) as DllGetClassObjectInvoker;
                        if (invoker != null)
                        {
                            object unknow;
                            int hr = invoker(clsid, UnknownId, out unknow);
                            if (hr >= 0)
                            {
                                IComClassFactory factory = unknow as IComClassFactory;
                                if (factory != null)
                                {
                                    object createdObject;
                                    factory.CreateInstance(null, UnknownId, out createdObject);
