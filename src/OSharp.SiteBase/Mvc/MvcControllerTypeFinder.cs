...
//  <last-date>2015-07-29 15:17</last-date>
// -----------------------------------------------------------------------
...
namespace OSharp.SiteBase.Mvc
{
    ...
    /// MVC控制器类型查找器
    /// </summary>
    public class MvcControllerTypeFinder : ITypeFinder
    {
        /// <summary>
        /// 初始化一个<see cref="MvcControllerTypeFinder"/>类型的新实例
        /// </summary>
        public MvcControllerTypeFinder()
        {
        ...
        public Type[] FindAll()
        {
            ...
            return assemblies.SelectMany(assembly =>
                assembly.GetTypes().Where(type =>
                    typeof(Controller).IsAssignableFrom(type) && !type.IsAbstract))
                .Distinct().ToArray();
        }
    }
}